...
RUN printf '%s\n' "Defaults:dietpi !requiretty" > /etc/sudoers.d/mupibox-dietpi && printf '%s\n' "dietpi ALL=(ALL) NOPASSWD: /usr/bin/pkill -f /usr/local/bin/mupibox/sleep_timer.sh, /bin/rm -f /tmp/.time2sleep, /usr/bin/nohup /usr/local/bin/mupibox/sleep_timer.sh *" >> /etc/sudoers.d/mupibox-dietpi && chmod 0440 /etc/sudoers.d/mupibox-dietpi
